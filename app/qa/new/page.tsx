  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (passed) {
        ...
        txHash = await certifyWrite([BigInt(lotID), certHash]);
      } else {
        /* For a fail, we pass the summary as the fail reason. */
        txHash = await failWrite([BigInt(lotID), summary.slice(0, 256)]);
      }
...
export default function QANewPage() {
  return (
    <>
      <Header />
      <Suspense fallback={<main className="px-6 py-10"><p>Loading...</p></main>}>
        <QANewContent />
      </Suspense>
    </>
  );
